...
class Balanced_propagation:
    ...
    def calculate_S_matrix(self, wl):
        self.calculate_losses(wl)                                 # Calculate propagation losses at current wavelength
        n = self.neff + self.dn * (wl - self.wl0)                 # Calculate the effective index at current wavelength
        beta = 2 * np.pi * n / wl                                 # Wave vector

        self.S = np.array([[np.exp(-1j * (beta * self.L * 1e4 + self.dphi_heater))
                            * np.exp(-self.alpha_neper * self.L), np.zeros(len(wl))],
                           [np.zeros(len(wl)), np.exp(-1j * (beta * (self.L * 1e4)))
...
class Unbalanced_propagation(Balanced_propagation):
    ...
    def calculate_S_matrix(self, wl):
        ...
        self.S = np.array(
            [[np.exp(-1j * (beta * (self.L * 1e4 + self.dL) + self.dphi_heater))
